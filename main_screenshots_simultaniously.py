...
def parse_url(url):
    ...
    '''
    capture_url_queue.put(url) # add url to screenshot list
...
def take_imgae_of_url(url):
    # add url to known pages of website
    img_path = capture_image_of_element(url, None)
    tup = (url, img_path)
    data_dict[DICT_ALL_PAGES].append(tup)

def screenshot_urls_from_queue():
    while threads_working:
        if not capture_url_queue.empty():
            url = capture_url_queue.get()
            take_imgae_of_url(url)
...
def create_threads_and_start_working(first_url, thread_num, sleep_time):
    ...
    # set flag to capture urls screenshots
    ...
    print 'STARTING SCREENSHOTS THREADS'
...
def main():
    ...
    # start
    create_threads_and_start_working(first_url=url, thread_num=10, sleep_time=5)
